...
```{r}
ggplot(data = weather_perth, aes(x = raintomorrow, y=month)) +
  geom_boxplot() 
```

```{r}
ggplot(data = weather_perth, aes(x = month, fill=raintomorrow)) +
  geom_histogram()
```

```{r}
ggplot(data = weather_perth, aes(x = humidity9am, fill=raintomorrow)) +
  geom_histogram()
```

```{r}
ggplot(data = weather_perth, aes(x = humidity9am, y=humidity3pm, color=raintomorrow)) +
  geom_point() + 
  geom_smooth() 
```
...
```{r}
ggplot(data = hotel_cancellations, aes(x = average_daily_rate, y=lead_time, color=is_canceled)) +
  geom_point() + 
  geom_smooth() 
```

```{r}
hotel_model1 <- stan_glm(
  is_canceled ~ lead_time, 
  data = hotel_cancellations, family = binomial,
...
4.  Use `exp(posterior_interval()` to interpret key predictors
    ...
    ```{r}
     exp(posterior_interval(hotel_model1, prob=0.80))
    ```
