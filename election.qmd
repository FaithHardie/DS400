...
![](images/posterior_model_quiz.png){fig-align="left" width="1033"}
...
```{r}
plot_beta_binomial(alpha = 45, beta = 55, y = 120, n = 200)
```
...
The prior model puts the probability between 90-100% certainty. The likelihood is between 10-30%. The prior model had much higher outcome before the scaled likelihood.

1.  Describe the posterior model in words. Does it more closely agree with the data (as reflected by the likelihood function) or the prior?

The posterior model puts the certainty between 70 and 80%. it more closely agrees with the prior than the likelihood function.

1.  Provide the specific `plot_beta_binomial()` code you would use to produce a similar plot.

```{r}
plot_beta_binomial(alpha = 100, beta = 5, y = 10, n = 40)
```
