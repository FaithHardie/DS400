...
```{r}
ggplot(data = weather_WU, aes(x = location, y=temp9am)) +
  geom_boxplot() 
  
```

```{r}
ggplot(data = weather_WU, aes(x = humidity9am, y=temp9am, color=location)) +
  geom_point() + 
  geom_smooth() 
```

```{r}
ggplot(data = subset(weather_WU, location == "Wollongong"),
       aes(x = windspeed9am, y = temp9am)) +
  geom_point() +
  geom_smooth() 
```

```{r}
ggplot(data = weather_WU, aes(x = temp3pm, fill=location)) +
  geom_histogram()
```
