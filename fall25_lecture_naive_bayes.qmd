...
```{r}
ggplot( data=penguins_bayes, aes(x=body_mass_g, y=flipper_length_mm, fill=species)) +
  geom_violin(alpha=0.4)+ geom_boxplot(width=0.2)
```

```{r}
ggplot( data=penguins_bayes, aes(x=body_mass_g, y=flipper_length_mm, fill=bill_length_mm)) +
  geom_point() +
  geom_smooth() +
  facet_wrap(~species) + 
...
```{r}
probability_adelie_given_below_average_weight <- 0.65
```
...
```{r}
our_penguin <- data.frame(bill_length_mm = 50, flipper_length_mm = 195, above_average_weight = "Under_average_weight")
```
...
```{r}
naive_model_2 <- naiveBayes(climate_change ~ science_is_honest + earth_sun + vaccines_are_safe, data = pulse_of_the_nation)

```

```{r}
our_person <- data.frame(science_is_honest = "Strongly Disagree", earth_sun = "false", vaccines_are_safe = "Strongly Disagree")
```

```{r}
predict(naive_model_2, newdata = our_person, type = "raw")
```
