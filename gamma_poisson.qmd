...
```{r}
plot_gamma(shape= 2.33, rate= 0.33, mean = TRUE, mode = TRUE)
```

1.  The most common value of λ is 10 and the mean is 12.

```{r}
plot_gamma(shape= 6, rate= 0.5, mean = TRUE, mode = TRUE)
```

1.  The most common value of λ is 5, and the variance is 3.

```{r}
#plot_gamma(shape= 6, rate= 1, mean = TRUE, mode = TRUE)
#incomplete
```
...
1.  Why is the Poisson model a reasonable choice for our data Yi?
    ...
    -   The poisson model is a reasonable choice for our data Yi is because we can see the mode, mean, and range. The world cup data is count data, not a percentage. We're looking at count over time.

    -   this generally fits our understanding of goals scored in a soccer game (very few.)
