...
```{r}
ggplot(data = football, aes(x = volume, fill=group)) +
  geom_histogram()
```

```{r}
ggplot(data = football, aes(x = years, y=volume, color=group)) +
  geom_point()+
  geom_smooth()
```

```{r}
ggplot(data = football, aes(x = group, y=volume, fill=group)) +
...
Chtgpt's first critique was that i should have used a beta-binomial model instead of gamma possion, which I originally did use and got good results for, so that critique made sense to me. It also said I should use a real world dataset, which I thought was funny since it created the dataset for me.

It provides this summary of its feedback: "Your Gamma–Poisson model is a good starting point if you think of homelessness as *counts/rates*, but since your data are **binary outcomes**, a **Beta–Binomial model or Bayesian logistic regression** would be more appropriate. Assumptions about Poisson variance and independence don’t perfectly match the reality of homelessness risk, which is clustered and overdispersed. For stronger applications, look at **HUD PIT counts or HMIS data** instead of synthetic data."
