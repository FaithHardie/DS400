...
```{r}
lm(percent_fair_or_poor_health~food_environment_index, 
   chr_data_simplified)
```
...
```{r}
food_model <- stan_glm(percent_fair_or_poor_health~food_environment_index, data = chr_data_simplified,
                       family = gaussian,
...
# Step 1: Summarize to state-level averages
state_summary <- chr_data_simplified %>%
  group_by(state) %>%
  summarize(avg_food_index = mean(food_environment_index, na.rm = TRUE))

plot_usmap(data = state_summary, values = "avg_food_index", color = "white") +
  scale_fill_continuous(
    low = "#edf8fb", high = "#006d2c",
    name = "Avg Food Index", label = scales::comma
  ) +
  theme(legend.position = "right") +
  labs(title = "Average Food Environment Index by State")
