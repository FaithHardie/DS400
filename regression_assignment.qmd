...
# YOUR CODE HERE
ggplot( data=penguins_bayes, aes(x=body_mass_g, y=flipper_length_mm, color=species)) +
  geom_point()+
  geom_smooth() +
  stat_cor()
...
# YOUR CODE HERE
ggplot( data=penguins_bayes, aes(x=body_mass_g, y=species,  fill=species)) +
...
1.  What is the correlation between flipper length and body mass?
    ...
    As body mass increases, flipper length appears to increase as well.

2.  Which species tends to be heaviest? Lightest?

    Gentoo appears to the heaviest species, and Adelie appears to be the lightest.

3.  Does the relationship between flipper length and body mass appear consistent across species?

    Yes, while the relationship seems stronger in the Gentoo species, the other two also seem to have a relationship between flipper length and body mass.

4.  Which predictors do you think will be most useful?

    I think when predicting body mass, species and flipper length will be the most useful.
:::
...
Species still matters after accounting for flipper size
:::
...
I think the simulated predictions capture the shape of actual data decently, but in the 4th quadrant tend to have trouble. The predictions are too wide then, but model 1 or 4 are the best. Model 3 misses an important curve.
:::
...
prediction_summary_cv(penguin_model_2, data = penguins_complete, k = 10)
...
prediction_summary_cv(penguin_model_3, data = penguins_complete, k = 10)
...
prediction_summary_cv(penguin_model_4, data = penguins_complete, k = 10)
...
# YOUR CODE HERE
## For future faith remember to add title
...
Around 1/3, a little over, of the articles in the dataset are fake.

2.  Do fake news articles use more exclamation points?

Yes, but with the limited data points for articles with exclamations, this is not conclusive.

3.  Do fake and real news differ in title length?

    fake news articles are longer.

4.  Is negative sentiment associated with fake news?

Yes, the box plot of negative sentiment typically shows that fake news articles have higher negative sentiment scores on average compared to real news articles, which means fake news tends to use more negative emotional language.
:::
...
1.  **title_words** (OR = 0.83 to 0.96): How does title length relate to fake vs real news?
    ...
    Articles with longer titles have 5-17 percent lower odds of being real news, Articles with longer titles are slightly more asscoiated with fake news.

2.  **negative** (OR = 0.60 to 0.88): How does negative sentiment relate to fake vs real news?

    Article with higher negative sentiment have 12 to 40 percent lower odds of being real news. This means negative sentiments have a medium association to fake news.

**Summary question:** Based on these odds ratios, what characteristics define fake news in this dataset?

From this dataset I can conclude that fake news articles are characterized by exlamantion points, high negative sentiment scores, and long titles..
:::
...
1.  What is the overall accuracy? Is this good?
    ...
    THis model is only 74% accurate, which is not that good, I think above 90%, maybe higher like 95% is the most ideal.

2.  What is the sensitivity? Are we good at identifying **real** news?

    Sensitivty is 0.92, which means the model is good a identifying real news.

3.  What is the specificity? Are we good at catching **fake** news?

    The specificity is 0.48, which means the model is bad at catching fake news.
...
4.  Which type of error is more problematic:
    ...
    False positives are more problematic because that would confirm misinformation as fact, which irl would cause decision makers and the public to come to incorrect conclusions.
:::
...
|---------------|---------------|---------------|---------------|---------------|
| 0.3 | 0.686 | 0.99 | 0.23 | Social media platforms |
| 0.5 | 0.747 | 0.922 | 0.483 | Balanced |
| 0.7 | 0.627 | 0.489 | 0.83 | Fact checking sites |

If would use a lower cut off for social media because its more lilely to identify real news, and can flag the post to let viewers know its real. I would use a higher cut off for a fact checking wesbit
:::
...
fake_news %>%
  ...
  geom_line(aes(group = .draw), alpha = 0.15) +
  geom_point(aes(y = as.numeric(type == "Real")), alpha = 0.3) +
  labs(title = "Predicted probability of real news using Title Length") +
  theme_minimal()
...
1.  The relationship between title length and the probability of **real** news?
    ...
    The plot shows that as title length increases, the probability of real news tends to decrease, showing a negative relationship.

2.  How does this relationship differ for articles with/without exclamation points?

    Articles with exclamation points show lower probabilities of being real news.

3.  Where is the model most/least certain?

    The model is least uncertain in title length, possibly because there are less data points.

    The model shows more uncertainty (wider spread of lines) at the extremes of title length, where there are fewer data points. In the middle range where most data points cluster, the predictions are more consistent.

4.  Does the visualization support your odds ratio interpretations?

    Yes, the visualization supports the odds ratio interpretation.
:::
